function cargarVistaFMatricula(itemID) {
    $$("views").addView({
        view: "tabview",
        id: itemID,
        height: 900,
        cells: [
            {
                ...
                header: "Pagamentos de Matr&iacute;cula", body: {
                    ...
                    rows: [
                        {
                                    ...
                                    cols: [
                                        {
                                                ...
                                                "onChange": function (newv, oldv) {
                                                    ...
                                                    if (this.getValue !== "") {
                                                        if (candidato_id !== "false") {
                                                            ...
                                                            webix.ui({
                                                                view: "window",
                                                                id: "id_win_matricula_add",
                                                                width: 600,
                                                                position: "center",
                                                                modal: true,
                                                                head: "Pagamento de Matr&iacute;cula",
                                                                body: webix.copy(formADDPagamentoMatricula)
                                                            }).show();
                                                            ...
                                                            $$("idDTFormPagamentoMatricula").clearAll();
                                                            $$("idDTFormPagamentoMatricula").load(BASE_URL + "CFinancas_Pagamentos_Pendientes_Candidatos/read_ncpXid_M?id=" + candidato_id/*this.getValue()*/);
                                                            //actualizar valor total a pagar
                                                            var envio = "cb=" + candidato_id; //this.getValue();
                                                            var r = webix.ajax().sync().post(BASE_URL + "CFinancas_Pagamentos_Pendientes_Candidatos/readXcb_valor_total_matricula", envio);
                                                            var total_pagar = r.responseText;
                                                            $$("idText_fpcValor_M").setValue(total_pagar);
                                                            $$("idText_fpcValor_M").disable();
                                                            //mandar este valor para que despues sea mandado para mFinancas_Inscricao_Comprobativo
                                                            $$("idText_id_M").setValue(candidato_id/*this.getValue()*/);
                                                        } else
                                            ...
                                            //readIDxBI
                                            view: "text", label: 'Localizar por BI', name: "cBI_Passaporte", id: "idText_cBI_Passaporte_M", labelPosition: "top",
                                            on: {
                                                "onChange": function (newv, oldv) {
                                                    //cargar el id seleccionado
                                    ...
                                    cols: [
                                        {
                                            view: "button", type: "standard", value: "Actualizar", width: 120, height: 50, click: function () {
                                                $$("idDTFMatricula").clearAll();
                                                $$("idDTFMatricula").load(BASE_URL + "CEstudantes/read_mat_financas");
                                                ...
                                                //limpar campos
                                                $$("idText_cBI_Passaporte_M").setValue("");
                                                $$("idCombo_CB").setValue("");
                                            }
                                        },
                                        {
                                            view: "button", type: "form", value: "Reimprimir Comprovativo", width: 120, height: 50, click: function () {
                                                var idSelecionado = $$("idDTFMatricula").getSelectedId(false, true);
                                                if (idSelecionado) {
                                                    //
                                                    var envio_candidato_id = "id=" + idSelecionado;
                                                    var rec = webix.ajax().sync().post(BASE_URL + "CFinancas_Pagamentos_Pendientes_Candidatos/read_estado_pagamento_estudantes_matricula", envio_candidato_id);
                                                    if (rec.responseText != "Mat.Aceite") {
                                                        webix.message({ type: "error", text: "Erro, Ainda n&atilde;o fez pagamento" });
                                                    } else
                                                        if (rec.responseText == "Mat.Aceite") {
                                                            var envio = "id=" + idSelecionado +
                                                                "&utilizadores_id=" + user_sessao;
                                                            var r = webix.ajax().sync().post(BASE_URL + "CFinancas_Matricula_Comprovativo_REIMP/imprimir", envio);
                                                            if (r.responseText == "true") {
                                                                webix.message("PDF criado com sucesso");
                                                                //Carregar PDF
                                                                webix.ui({
                                                                    view: "window",
                                                                    id: "idWinPDFM_Comprobativo",
                                                                    height: 600,
                                                                    width: 700,
                                                                    left: 50, top: 50,
                                                                    move: true,
                                                                    ...
                                                                    head: {
                                                                        view: "toolbar", cols: [
                                                                            ...
                                                                            { view: "button", label: 'X', width: 50, align: 'right', click: function () { $$('idWinPDFM_Comprobativo').close(); } }
                                                                        ]
                                                                    },
                                                                    body: {
                                                                        //template:"Some text"
                                                                        template: '<div id="idPDFM_Comprobativo" style="width:690px;  height:590px"></div>'
                                                                    }
                                                                }).show();
                                                                PDFObject.embed("../../relatorios/Financas_Matricula_Comprovativo_REIMP.pdf", "#idPDFM_Comprobativo");
                                                    ...
                                                    if (rec.responseText == "Espera de Pagamento") {
                                                        ...
                                                        if (rec.responseText == "Inscrição aceite") {
                                                            ...
                                                            if (r.responseText == "true") {
                                                                ...
                                                                $$("idDTFMatricula").clearAll();
                                                                $$("idDTFMatricula").load(BASE_URL + "cCandidatos/readDInscricao_Financas");
                                                            } else {
                                                                webix.message({ type: "error", text: "Erro ao carregar dados" });
                                                            }
                                                        }
                            ...
                            columns: [
                                ...
                                {
                                    id: "eEstado_Matricula1", header: ["Estado", { content: "selectFilter" }], width: 170, sort: "string",
                                    template: function (obj) {
                                        if (obj.eEstado_Matricula1 == "Mat.Esp.Pag")
                                            return "<span style='color:red;'>" + obj.eEstado_Matricula1 + "</span>";
                                        else
                                            if(obj.eEstado_Matricula1 == "Mat.Aceite")
                                                return "<span style='color:green;'>" + obj.eEstado_Matricula1 + "</span>";
                                    },
                                },
                            ...
                            url: BASE_URL + "CEstudantes/read_mat_financas",
                            pager: "pagerCDadosMatricula"
                        }, {
                            view: "pager", id: "pagerCDadosMatricula",
...
var formADDPagamentoMatricula = {
    view: "form",
    id: "idformADDPagamentoMatricula",
    borderless: true,
    elements: [
        {
            rows: [
                {
                    ...
                    columns: [
                        ...
                        { id: "ncPreco_Matricula", editor: "text", header: "Pre&ccedil;o Mat.", width: 120, sort: "int" },
                    ],
                    url: BASE_URL + "CFinancas_Pagamentos_Pendientes_Candidatos/mread_ncpXid_M",
        ...
        },
        {
            cols: [
                {},
                { view: "text", label: 'id', name: "id", id: "idText_id_M", hidden: true, validate: "isNotEmpty", validateEvent: "blur" },
                { view: "text", label: 'Valor a Pagar (Kz)', name: "fpcValor", id: "idText_fpcValor_M", readonly: true, validate: "isNotEmpty", validateEvent: "blur" },
            ]
        },
        {
            cols: [
                {
                    view: "button", value: "Salvar", click: function () {
                        //criar PDF
                        var idSelecionado = $$("idText_id_M").getValue();
                        ...
                        if (idSelecionado && bancNome && contNumero) {
                            var envio = "id=" + idSelecionado +
                                "&total_pagar=" + $$("idText_fpcValor_M").getValue() +
                                "&bancNome=" + bancNome +
                                "&contNumero=" + contNumero +
                                "&ffpNome=" + ffpNome +
                                "&fpcRefPagamento=" + $$("idText_fpcRefPagamento").getValue() +
                                "&utilizadores_id=" + user_sessao;
                            var r = webix.ajax().sync().post(BASE_URL + "CFinancas_Matricula_Comprovativo/imprimir", envio);
                            if (r.responseText == "true") {
                                webix.message("PDF criado com sucesso");
                                //Carregar PDF
                                webix.ui({
                                    ...
                                    head: {
                                        view: "toolbar", cols: [
                                            { view: "label", label: "Finan&ccedil;as Comprovativo de Matr&iacute;cula" },
                                            { view: "button", label: 'X', width: 50, align: 'right', click: function () { $$('idWinPDFM_Comprobativo').close(); } }
                                        ]
                                    },
                                    body: {
                                ...
                                PDFObject.embed("../../relatorios/Financas_Matricula_Comprovativo.pdf", "#idPDFM_Comprobativo");
                                ...
                                if ($$("idText_cBI_Passaporte_M").getValue() !== "") {
                                    $$("idText_cBI_Passaporte_M").setValue("");
                                }
                                $$("id_win_matricula_add").close();
