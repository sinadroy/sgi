function cargarVistaDeclaracaoSemNotasMestrado(itemID) {
    var f = new Date();
    ...
    $$("views").addView({
        ...
        cells: [
            {
                header: "Declaração Sem Nota", body: {
                    rows: [{
                        view: "form", scroll: false,
                        cols: [
                            {
                                view: "button", type: "form", value: "Imprimir", width: 100, click: function () {
                                    var idSelecionado = $$("idDTEdDeclaracaoSemNotasMestrado").getSelectedId(false, true);
                                    //cargar dados necesarios para enviar para imprimir
                                    var record = $$("idDTEdDeclaracaoSemNotasMestrado").getItem(idSelecionado);
                                    ...
                                    if (idSelecionado) {
                                        ...
                                        var r = webix.ajax().sync().post(BASE_URL + "Csecretaria_declaracao_sem_notas_mestrado/imprimir", envio);
                                        if (r.responseText == "true") {
                                            ...
                                            PDFObject.embed("../../relatorios/secretaria_declaracao_sem_nota_mestrado.pdf", "#idPDFDSN");

                                            $$("idDTEdDeclaracaoSemNotasMestrado").clearAll();
                                            $$("idDTEdDeclaracaoSemNotasMestrado").load(BASE_URL + "Cdocumentos_pendientes/read_mestrado");
                            ...
                            view: "datatable",
                            id: "idDTEdDeclaracaoSemNotasMestrado",
                            select: true,
                            editable: false,
                            columns: [
                            ...
                            url: BASE_URL + "Cdocumentos_pendientes/read_mestrado",
                            pager: "pagerDocumentos_Pendientes_Mestrado"
                        }, {
                            view: "pager", id: "pagerDocumentos_Pendientes_Mestrado",
