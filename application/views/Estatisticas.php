...
<html>
   ...
   <!-- <link rel="stylesheet" href="http://cdn.webix.com/edge/webix.css" type="text/css"> 
    ...
    <body>
        ...
        <!-- SUB-MODULOS -->
        <script src=<?php echo $web_dir."MODULOS/Estatisticas/EInscricao.js";?> type="text/javascript"></script>
        <script src=<?php echo $web_dir."MODULOS/Estatisticas/EMatricula.js";?> type="text/javascript"></script>
        <script src=<?php echo $web_dir."MODULOS/Estatisticas/EAproveitamento.js";?> type="text/javascript"></script>
        <script src=<?php echo $web_dir."MODULOS/Estatisticas/ERelatorios.js";?> type="text/javascript"></script>
       ...
       <script type="text/javascript" charset="utf-8">
        ...
        webix.ui({
            ...
            rows:[	
                {view:"toolbar", cols:[
                    ...
                    {},
                    { view:"label", label:"ESTATÍSTICAS",width:300},
                    {},
		...
		{ cols:[
			...
			type:{
                        ...
                        url: BASE_URL+"CSubModulos/read?modulo=11&usuario="+user_sessao,
			select:true,
			on:{
                    ...
                    rows:[
                        ...
                        { id:"views", cells:[
                            {view:"template", id:"tpl", height:900,template:"Vem-Bindo ao Sub-Sistema de Estatisticas."}
                        ]}
                    ]}
                ]}
            ]
        ...
        function open_new_tab(id){
            var item = $$('menuEstatisticas').getItem(id);
            //add tab
            if(!$$(item.id)){
                if(item.smCodigo==='1101'){
                    cargarVistaEInscricao(item.smCodigo);
                }
                if(item.smCodigo==='1102'){
                    cargarVistaEMatricula(item.smCodigo);
                }
                if(item.smCodigo==='1103'){
                    cargarVistaAproveitamento(item.smCodigo);
                }
                if(item.smCodigo==='1104'){
                    cargarVistaRelatorios(item.smCodigo);
                }
