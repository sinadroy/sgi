<?php
  class Mmultas_propina_mestrado extends CI_Model{
      
      function mread(){
          $this->db->select('multas_propina_mestrado.id,mp_data_inicio,mp_data_fin,mp_porciento,mesnome');
          $this->db->from('multas_propina_mestrado');
          $this->db->join('meses_propina_mestrado', 'multas_propina_mestrado.meses_propina_mestrado_id = meses_propina_mestrado.id');
      ...
      function mreadIdXMes($mesNome){
          $this->db->select('meses_propina_mestrado.id');
          $this->db->from('multas_propina_mestrado');
          $this->db->join('meses_propina_mestrado', 'multas_propina_mestrado.meses_propina_mestrado_id = meses_propina_mestrado.id');
          $this->db->where('meses_propina_mestrado.mesnome', $mesNome);
          $consulta = $this->db->get();
          foreach ($consulta->result() as $row) {
            return $row->id;
          }
      ...
      function mread_porciento($mes_a_pagar, $ano_a_pagar){
          ...
          $this->db->select('multas_propina_mestrado.id, mp_porciento, mp_data_inicio, mp_data_fin');
          $this->db->from('multas_propina_mestrado');
          $this->db->join('meses_propina_mestrado', 'multas_propina_mestrado.meses_propina_mestrado_id = meses_propina_mestrado.id');
          $this->db->where('meses_propina_mestrado.mesnome', $mes_a_pagar);
          $consulta = $this->db->get();
      ...
      function mupdate($id,$mp_data_inicio,$mp_data_fin,$mp_porciento,$meses_propina_id){
            $dados = array('mp_data_inicio'=>$mp_data_inicio, 'mp_data_fin'=>$mp_data_fin, 'mp_porciento'=>$mp_porciento,
                'meses_propina_mestrado_id'=>$meses_propina_id);
            if($this->db->update('multas_propina_mestrado', $dados, array('id' => $id))){
                return true;
            }else
                return false;
      } 
      ...
      function minsert($mp_data_inicio,$mp_data_fin,$mp_porciento,$meses_propina_id){
        $dados = array('mp_data_inicio'=>$mp_data_inicio,'mp_data_fin'=>$mp_data_fin,'mp_porciento'=>$mp_porciento, 'meses_propina_mestrado_id'=>$meses_propina_id);
        if($this->db->insert('multas_propina_mestrado', $dados))
        {
            return true;
        }else{
            return false;
    ...
    function mdelete($id) {
        if($this->db->delete('multas_propina_mestrado', array('id' => $id)))  
            return true;
        else
            return false; 
    }     
