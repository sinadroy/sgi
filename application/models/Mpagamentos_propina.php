<?php
  class Mpagamentos_propina extends CI_Model{
      ...
      */
      function mreadXvalor_propina($bi, $mes_a_pagar, $ano_a_pagar){
          ...
          $porc = $this->mmultas_propina->mread_porciento($mes_a_pagar, $ano_a_pagar);
      ...
      function mExiste_Pagamento($bi,$alAno){
          $this->db->select('Pagamentos_Propina.id');
          $this->db->from('Pagamentos_Propina');
          $this->db->join('Estudantes', 'Pagamentos_Propina.Estudantes_id = Estudantes.id');
          $this->db->join('Candidatos', 'Estudantes.Candidatos_id = Candidatos.id');
          $this->db->join('anos_lectivos', 'Pagamentos_Propina.anos_lectivos_id = anos_lectivos.id');
          ...
          if($this->db->count_all_results() > 0)
            return true;
              ...
              foreach ($this->Mmeses_propina->mreadXactivos() as $row) {
     ...
     function mdt_divida($al,$n,$c,$p,$m,$bi){
          ...
          $this->db->where('Meses_Propina_id', $m);
          $this->db->where('Candidatos.cBI_Passaporte', $bi);
          if($this->db->count_all_results() > 0)
            return true;
          else
    ...
    function mupdate($id,$ppData,$ppHora,$ppValor,$utilizadores_id,$Financas_Forma_Pagamento_id,$Financas_Contas_id,$fpcRefPagamento,$cNome,$bi,$user){
        ...
        if($this->db->update('Pagamentos_Propina', $dados, array('id' => $id))){
            $this->load->model('MAuditorias_Financas');
            $this->MAuditorias_Financas->minsert("Inserir:Pagamento","Financa","Pag.Propinas",$user,"Estudante:".$cNome.' BI:'.$bi.' inserido com sucesso');
            return true;
        }else
            return false;
    }
    ...
    function minsert($anos_lectivos_id,$ppData,$ppHora,$ppValor,$Meses_Propina_id,$Estudantes_id,$utilizadores_id,$Financas_Forma_Pagamento_id,$Financas_Contas_id){
        $dados = array('anos_lectivos_id'=>$anos_lectivos_id,'ppData'=>$ppData,'ppHora'=>$ppHora,'ppValor'=>$ppValor,'Meses_Propina_id'=>$Meses_Propina_id,
        ...
        if($this->db->insert('Pagamentos_Propina', $dados))
        {
            return true;
        }else{
            return false;
