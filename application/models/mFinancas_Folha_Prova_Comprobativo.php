<?php
  class MFinancas_Folha_Prova_Comprobativo extends CI_Model{
    
    var $hpdf = '';

    public function criarPdf($id,$fc_data,$fc_hora,$fc_ref_pag,$fc_valor,$ffpNome,$contNumero,$anos_lectivos_id,$bi,$Estudantes_id,$user)
    {
        $this->load->model('mcandidatos');
        $Candidatos_id = $this->mcandidatos->mreadIDxBI($bi);
        ...
        $uid = $this->mutilizadores->mreadXnome($user);
        ...
        //$contador = 1;
        $this->load->model('mEstudantes');
        $this->load->model('MFinancas_folha_prova');
        $this->load->model('MNiveisCursos');
        //$bi = $this->getBI($id);
        foreach ($this->MFinancas_folha_prova->mreadX_id($id) as $value) {
            $cNome = $value['cnome'];
            $cNomes = $value['cnomes'];
            $cApelido = $value['capelido'];
            $cBI_Passaporte = $value['cbi_passaporte'];
            $nNome = $this->MNiveisCursos->mread_n_x_ide($Candidatos_id);
            $curso = $this->MNiveisCursos->mread_c_x_ide($Candidatos_id);
            $pNome = $this->MNiveisCursos->mread_p_x_ide($Candidatos_id);
            ...
            <page>
                <div align="center">
                    ...
                    <table align="center" border="1">
                        <tr ><td border="0" align="center" width="600"> <h3>Finan&ccedil;as Comprovativo de Folha de Prova.</h3><br> </td></tr>
                    </table>
                    <br>
                    <table>
                ...
                <div>
                    <table>
                        <tr ><td border="0" align="left"> Funcionario: '.$user.'</td></tr>
                    </table>
        ...
        $this->hpdf->Output('relatorios/Financas_Folha_Prova_Comprovativo.pdf','F');
        echo "true";
    }       
  }
