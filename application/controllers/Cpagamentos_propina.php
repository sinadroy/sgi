<?php
class Cpagamentos_propina extends CI_Controller {
    
    public function read(){
        $this->load->model('Mpagamentos_propina');
        echo json_encode($this->Mpagamentos_propina->mread());
    }
    ...
    public function readXvalor_propina(){
        ...
        $this->load->model('Mpagamentos_propina');
        echo $this->Mpagamentos_propina->mreadXvalor_propina($bi, $mes_a_pagar, $ano_a_pagar);
    }
    public function Existe_Pagamento() {
        $bi = $this->input->post('bi');
        $alAno = $this->input->post('alAno');
        $this->load->model('Mpagamentos_propina');
        echo ($this->Mpagamentos_propina->mExiste_Pagamento($bi,$alAno))?"true":"false";
    }
    //cancelar_pagamento
    ...
    public function crud(){
        ...
        $Meses_Propina_id = $this->Mmeses_propina->mGetID($mesNome);
        ...
        if ($webix_operation == "insert"){
            if($this->Mpagamentos_propina->minsert($id,$anos_lectivos_id,$ppData,$ppHora,$ppValor,$Meses_Propina_id,$Estudantes_id,$utilizadores_id))
                echo "true";
            else
                echo "false";
        } else if ($webix_operation == "update"){
            if($this->Mpagamentos_propina->mupdate($id,$ppData,$ppHora,$ppValor,$utilizadores_id,$Financas_Forma_Pagamento_id,$Financas_Contas_id,$fpcRefPagamento,$cNome,$bi,$user))
                echo "true"; 
            else
                echo "false";
