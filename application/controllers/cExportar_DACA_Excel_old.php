...
class CExportar_DACA_Excel extends CI_Controller {
	...
	public function Dados_Inscricao(){
		...
		*/
		$objPHPExcel->getActiveSheet()->setCellValue('A1', "PROFISSÃO");
		$objPHPExcel->getActiveSheet()->setCellValue('B1', "TRABALHADOR");
		$objPHPExcel->getActiveSheet()->setCellValue('C1', "TIPO INSTITUIÇÃO LABORAL");
		$objPHPExcel->getActiveSheet()->setCellValue('D1', "ORGANISMO TUTELA");
		$objPHPExcel->getActiveSheet()->setCellValue('E1', "LOCAL TRABALHO");
...
/*
		...
		foreach($this->MCandidatos->mreadDPRO() as $row){
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$rowExcel, $row['proNome']);
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$rowExcel, $row['trabNome']);
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$rowExcel, $row['tilNome']);
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$rowExcel, $row['otNome']);
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$rowExcel, $row['dlLocal_Trabalho']);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$rowExcel, $row['dlCargo']);
			$rowExcel++;
		}
		*/
		...
		foreach($this->MCandidatos->mreadDACA() as $row){
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$rowExcel, $row['paFormacao']);
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$rowExcel, $row['provFormacao']);
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$rowExcel, $row['hlfNome']);
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$rowExcel, $row['efNome']);
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$rowExcel, $row['Ano']);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$rowExcel, $row['Media']);
			$rowExcel++;
		}
/*
		$rowExcel = 3;
